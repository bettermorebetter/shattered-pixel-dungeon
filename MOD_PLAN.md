...
- [x] **1.1a** 적 데미지 정보 — 살펴보기 창(`WndInfoMob`)에 "공격력 X~Y / 정확성" 표시 ✅
...
- [ ] **3.4** 적 회피 수치 정규화
- [ ] **3.5** 데미지 범위(곡선) 재설계 — 현재 거의 모든 적이 `NormalIntRange(1, max)`로 최소값 1. 참고: `NormalIntRange`는 중앙값에 몰리는 삼각분포라 보이는 것보다 덜 널뜀(min/max는 드뭄). 강한 적일수록 min을 끌어올리고 min/max 간격을 좁혀 "더 묵직하고 예측 가능한 위협"으로. 데미지를 이제 표시(1.1)하므로 "정보 제공" 원칙과도 시너지(슬더스식 계획 가능). 방어구 DR(`drRoll`)과의 상호작용 고려 필요.
